...
def model_trainer(models, candidate_models_idx, retrained_models_path, cm_plots_path, split_data_path, matGenData):
    trained_models = []
    total_training_times = []

    if matGenData:
        y_train = to_categorical(np.load(os.path.join(split_data_path, "Y_train.npy")))
        y_val = to_categorical(np.load(os.path.join(split_data_path, "Y_val.npy")))

    else:
        y_train = np.load(os.path.join(split_data_path, "Y_train.npy"))
        y_val = np.load(os.path.join(split_data_path, "Y_val.npy"))        
    ...
    for idx, model in enumerate(models):
            ...
            model.fit(
                ...
                batch_size=1024,
                epochs=100,
                validation_data=(np.load(os.path.join(split_data_path, "X_val.npy")), y_val),
                verbose=1,
...
def main():  
    ...
    parser.add_argument(
        'matlab_gen_data',
        help="matlab generated data or not"
    )
