...
class State(object):
    ...
    def to_tensorflow_layer(self): 
        if self.terminate:
            ...
            return tf.keras.layers.Convolution2D(filters=self.filter_depth, kernel_size=(self.filter_size, 1),
                                                 strides=(self.stride, 1), kernel_initializer='he_uniform',
            ...
            return tf.keras.layers.AveragePooling2D(pool_size=(self.filter_size, 1), strides=(self.stride, 1))
        elif self.layer_type == 'flatten':
            return tf.keras.layers.Flatten()
        elif self.layer_type == 'max_pool':
            return tf.keras.layers.MaxPooling2D(pool_size=self.filter_size, strides=(self.stride, 1))
