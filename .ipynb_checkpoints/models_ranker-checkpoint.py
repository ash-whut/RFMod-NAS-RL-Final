...
def best_model_updater(dataframe, idx: int, best_models: list, cache_limit: int) -> list:
    current_model_score = (dataframe.iloc[idx]['best_accuracy'] - 0.25)**3 / dataframe.iloc[idx]['trainable_parameters']**0.25
...
def model_ranker(file_path, cache_limit: int) -> None:
...
model_ranker('./metaqnn/learner_logs_old/replay_database.csv', 5)